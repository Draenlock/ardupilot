...
public:
    ...
    /* Do not allow copies */
    AP_OACSA(const AP_OACSA &other) = delete;
    AP_OACSA &operator=(const AP_OACSA&) = delete;
    ...
    enum AP_OACSA_State : uint8_t {
        CSA_STATE_NOT_REQUIRED = 0,
        CSA_STATE_ERROR,
        CSA_STATE_SUCCESS
    };
...
private:
    ...
    enum class AP_OACSA_Error : uint8_t {
        CSA_ERROR_NONE = 0,
        CSA_ERROR_OUT_OF_MEMORY,
        CSA_ERROR_OVERLAPPING_POLYGON_POINTS,
        CSA_ERROR_FAILED_TO_BUILD_INNER_POLYGON,
        CSA_ERROR_OVERLAPPING_POLYGON_LINES,
        CSA_ERROR_FENCE_DISABLED,
        CSA_ERROR_TOO_MANY_FENCE_POINTS,
        CSA_ERROR_NO_POSITION_ESTIMATE,
        CSA_ERROR_COULD_NOT_FIND_PATH
    };

    // return error message for a given error id
    const char* get_error_msg(AP_OACSA_Error error_id) const;

    // report error to ground station
    void report_error(AP_OACSA_Error error_id);
    ...
    bool create_inclusion_polygon_with_margin(float margin_cm, AP_OACSA_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_exclusion_polygon_with_margin(float margin_cm, AP_OACSA_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_exclusion_circle_with_margin(float margin_cm, AP_OACSA_Error &err_id);
    ...
    // returns true on success.  returns false on failure and err_id is updated
    bool create_fence_visgraph(AP_OACSA_Error &err_id);
    ...
    bool calc_shortest_path(const Location &origin, const Location &destination, AP_OACSA_Error &err_id);
