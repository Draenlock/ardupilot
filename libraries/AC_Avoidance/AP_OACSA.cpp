...
AP_OACSA::AP_OACSA() :
        ...
        _exclusion_circle_pts(OA_CSA_EXPANDING_ARRAY_ELEMENTS_PER_CHUNK),
        _short_path_data(OA_CSA_EXPANDING_ARRAY_ELEMENTS_PER_CHUNK),
        _path(OA_CSA_EXPANDING_ARRAY_ELEMENTS_PER_CHUNK)
{
}
...
AP_OACSA::AP_OACSA_State AP_OACSA::update(const Location &current_loc, const Location &destination, Location& origin_new, Location& destination_new)
{
    ...
    struct Location ekf_origin {};
    {
        ...
        if (!AP::ahrs().get_origin(ekf_origin)) {
            AP::logger().Write_OACSA(CSA_STATE_NOT_REQUIRED, 0, 0, 0, destination, destination);
            return CSA_STATE_NOT_REQUIRED;
        }
    }
    ...
    if (!_inclusion_polygon_with_margin_ok) {
        ...
        if (!_inclusion_polygon_with_margin_ok) {
            ...
            AP::logger().Write_OACSA(CSA_STATE_ERROR, (uint8_t)error_id, 0, 0, destination, destination);
            return CSA_STATE_ERROR;
        }
    }
    ...
    if (get_shortest_path_point(_path_idx_returned, dest_pos)) {
        ...
        // log success
        AP::logger().Write_OACSA(CSA_STATE_SUCCESS, 0, _path_idx_returned, _path_numpoints, destination, destination_new);
        return CSA_STATE_SUCCESS;
    }
...
bool AP_OACSA::some_fences_enabled() const
{
...
const char* AP_OACSA::get_error_msg(AP_OACSA_Error error_id) const
{
    switch (error_id) {
    case AP_OACSA_Error::CSA_ERROR_NONE:
        return "no error";
        break;
    case AP_OACSA_Error::CSA_ERROR_OUT_OF_MEMORY:
        return "out of memory";
        break;
    case AP_OACSA_Error::CSA_ERROR_OVERLAPPING_POLYGON_POINTS:
        return "overlapping polygon points";
        break;
    case AP_OACSA_Error::CSA_ERROR_FAILED_TO_BUILD_INNER_POLYGON:
        return "failed to build inner polygon";
        break;
    case AP_OACSA_Error::CSA_ERROR_OVERLAPPING_POLYGON_LINES:
        return "overlapping polygon lines";
        break;
    case AP_OACSA_Error::CSA_ERROR_FENCE_DISABLED:
        return "fence disabled";
        break;
    case AP_OACSA_Error::CSA_ERROR_TOO_MANY_FENCE_POINTS:
        return "too many fence points";
        break;
    case AP_OACSA_Error::CSA_ERROR_NO_POSITION_ESTIMATE:
        return "no position estimate";
        break;
    case AP_OACSA_Error::CSA_ERROR_COULD_NOT_FIND_PATH:
...
void AP_OACSA::report_error(AP_OACSA_Error error_id)
{
    // report errors to GCS every 5 seconds
    uint32_t now_ms = AP_HAL::millis();
    if ((error_id != AP_OACSA_Error::CSA_ERROR_NONE) &&
        ((error_id != _error_last_id) || ((now_ms - _error_last_report_ms) > OA_CSA_ERROR_REPORTING_INTERVAL_MS))) {
        const char* error_msg = get_error_msg(error_id);
        gcs().send_text(MAV_SEVERITY_CRITICAL, "CSA: %s", error_msg);
...
bool AP_OACSA::check_inclusion_polygon_updated() const
{
...
bool AP_OACSA::create_inclusion_polygon_with_margin(float margin_cm, AP_OACSA_Error &err_id)
{
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
        err_id = AP_OACSA_Error::CSA_ERROR_FENCE_DISABLED;
        return false;
    }
    ...
    for (uint8_t i = 0; i < num_inclusion_polygons; i++) {
        ...
        if (!_inclusion_polygon_pts.expand_to_hold(_inclusion_polygon_numpoints + num_points)) {
            err_id = AP_OACSA_Error::CSA_ERROR_OUT_OF_MEMORY;
            return false;
        }
        ...
        for (uint16_t j = 0; j < num_points; j++) {
            ...
            if (before_pt.is_zero() || after_pt.is_zero() || (before_pt == after_pt)) {
                err_id = AP_OACSA_Error::CSA_ERROR_OVERLAPPING_POLYGON_POINTS;
                return false;
            }
            ...
            if (Polygon_outside(_inclusion_polygon_pts[next_index], boundary, num_points)) {
                ...
                if (Polygon_outside(_inclusion_polygon_pts[next_index], boundary, num_points)) {
                    ...
                    err_id = AP_OACSA_Error::CSA_ERROR_OVERLAPPING_POLYGON_LINES;
                    return false;
                }
            }
        }
...
bool AP_OACSA::check_exclusion_polygon_updated() const
{
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
        return false;
...
// returns true on success.  returns false on failure and err_id is updated
bool AP_OACSA::create_exclusion_polygon_with_margin(float margin_cm, AP_OACSA_Error &err_id)
{
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
        err_id = AP_OACSA_Error::CSA_ERROR_FENCE_DISABLED;
...
// returns true if changed
bool AP_OACSA::check_exclusion_circle_updated() const
{
...
// returns true on success.  returns false on failure and err_id is updated
bool AP_OACSA::create_exclusion_circle_with_margin(float margin_cm, AP_OACSA_Error &err_id)
{
    // exit immediately if fence is not enabled
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
...
uint16_t AP_OACSA::total_numpoints() const
{
...
bool AP_OACSA::get_point(uint16_t index, Vector2f &point) const
{
    // sanity check index
...
bool AP_OACSA::intersects_fence(const Vector2f &seg_start, const Vector2f &seg_end) const
{
...
bool AP_OACSA::create_fence_visgraph(AP_OACSA_Error &err_id)
{
    // exit immediately if fence is not enabled
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr) {
    ...
    if (total_numpoints() >= OA_CSA_POLYGON_SHORTPATH_NOTSET_IDX) {
        err_id = AP_OACSA_Error::CSA_ERROR_TOO_MANY_FENCE_POINTS;
        return false;
    }
...
void AP_OACSA::update_visible_node_distances(node_index curr_node_idx)
{
    // sanity check
...
bool AP_OACSA::find_node_from_id(const AP_OAVisGraph::OAItemID &id, node_index &node_idx) const
{
    switch (id.id_type) {
...
bool AP_OACSA::find_closest_node_idx(node_index &node_idx) const
{
...
bool AP_OACSA::calc_shortest_path(const Location &origin, const Location &destination, AP_OACSA_Error &err_id)
{
    ...
    if (!origin.get_vector_xy_from_origin_NE(origin_NE) || !destination.get_vector_xy_from_origin_NE(destination_NE)) {
        err_id = AP_OACSA_Error::CSA_ERROR_NO_POSITION_ESTIMATE;
        return false;
    }
    ...
    _short_path_data[1] = {{AP_OAVisGraph::OATYPE_DESTINATION, 0}, false, OA_CSA_POLYGON_SHORTPATH_NOTSET_IDX, FLT_MAX};
    ...
    for (uint8_t i=0; i<total_numpoints(); i++) {
        _short_path_data[_short_path_data_numpoints++] = {{AP_OAVisGraph::OATYPE_INTERMEDIATE_POINT, i}, false, OA_CSA_POLYGON_SHORTPATH_NOTSET_IDX, FLT_MAX};
    }
    ...
    while (true) {
        ...
        if ((_short_path_data[nidx].distance_from_idx == OA_CSA_POLYGON_SHORTPATH_NOTSET_IDX) ||
...
bool AP_OACSA::get_shortest_path_point(uint8_t point_num, Vector2f& pos)
{
